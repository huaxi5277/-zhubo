...
<script>
	...
	export default {
		...
		methods: {
			...
			joinHistoryMsg() {
				...
				let join = () => {
					...
					// 		"type": ram > 0.5 ? 1 : 0, // 此为消息类别，设 1 为发出去的消息，0 为收到对方的消息,
					// 		"pic": ram > 0.5 ? this.chat_current_avatar : this.chat_current_friend // 头像
					// 	})
					// }
					...
					this.request({
						url : interfaces.chat_list_find,
						method : "POST",
						data : {
							chat_current_avatar : id,
							chat_current_friend : this.chat_current_friend
						},
						success : (res)=>{
							arr = res
						},
			...
			send() {
				...
				setTimeout(() => {
					...
					let info = {
						send_id: id,
						recover_id: this.chat_friend_id,
						msg: this.content
					}
		...
		onShow() {
			const id = uni.getStorageSync("id") // 当前用户的id 
			// 直接连接后端服务器
			...
			// 去后端查找 当前用户的头像 
			// this.request({
			// 	url: interfaces.chat_current_avarar_and_friend,
			// 	method: "POSt",
			// 	data: {
			// 		current_user_id: id,
			// 		friend_user_id: this.chat_friend_id
			// 	},
			// 	success: (res) => {
			// 		// 这里面 我拿着 头像
			// 		this.chat_current_avatar = res.current_user_id
			// 		this.chat_current_friend = res.friend_user_id
			// 	}
			// })
			...
			this.socket.on('receiveMsg', (data) => {
				let data_msg = {
					...
					"content": data,
					"type": 0,
					"pic": this.chat_current_friend
				}
				this.talkList.push(data_msg);
				
			})
		},
		onLoad(options) {
			this.chat_friend_id = options.chat
		}
	}
</script>
